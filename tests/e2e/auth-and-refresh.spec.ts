...
const authenticate = async (
  ...
  await page.getByPlaceholder('dev.patel@esscompany.com').pressSequentially(email, { delay: 30 });
...
test.describe('OWNER role', () => {
  test('login → /owner navigation (documents hardcoded redirect)', async ({ page }) => {
    ...
    await page.waitForURL(/\/ess-crm\/owner/, { timeout: 10000 });
  });

  test('direct URL access to /finance', async ({ page }) => {
    await authenticate(page, 'owner@test.com');
    const res = await page.goto('/ess-crm/finance');
  ...
  test('logout invalidates session', async ({ page }) => {
    ...
    const sessionAfter = await page.request.get('/ess-crm/api/auth/session');
...
test.describe('Anonymous access', () => {
  ...
  test('/portal/[valid-token] → ALLOWED (client portal is public)', async ({ page }) => {
    const res = await page.goto('/ess-crm/portal/test-portal-token-123');
