...
test.describe('Employee Management Feature (Post-Audit)', () => {
  ...
  test('Anonymous users are denied access to API', async ({ request }) => {
    const res = await request.post('/ess-crm/api/employees', {
      data: { name: 'Test', email: 'test@esscompany.com', password: '123' }
    });
  ...
  test('SALES cannot access employee API', async ({ page }) => {
    ...
    await page.getByPlaceholder('e.g. dev.patel@esscompany.com or EMP-004').fill('karan.sales@esscompany.com');
    ...
    const res = await page.request.post('/ess-crm/api/employees', {
      data: { name: 'Test', email: 'test_sales@esscompany.com', password: '123' }
    });
  ...
  test('EMPLOYEE cannot access employee API', async ({ page }) => {
    await page.goto('/ess-crm/login');
    await page.getByPlaceholder('e.g. dev.patel@esscompany.com or EMP-004').fill('dev.patel@esscompany.com');
    await page.getByPlaceholder('Enter your password').fill('password123');
    ...
    const res = await page.request.post('/ess-crm/api/employees', {
      data: { name: 'Test', email: 'test_emp@esscompany.com', password: '123' }
    });
    expect(res.status()).toBe(403);
  });
  ...
  test('OWNER can manage employees and role escalation is blocked', async ({ page }) => {
    // Login as owner
    await page.goto('/ess-crm/login');
    await page.getByPlaceholder('e.g. dev.patel@esscompany.com or EMP-004').fill('owner@esscompany.com');
    await page.getByPlaceholder('Enter your password').fill('password123');
    ...
    const email = `new_emp_${Date.now()}@esscompany.com`;
    
    // 1. Role escalation prevention via direct API call (simulating malicious frontend)
    const escalatedResponse = await page.request.post('/ess-crm/api/employees', {
      data: {
        name: 'Hacker',
        email: email,
